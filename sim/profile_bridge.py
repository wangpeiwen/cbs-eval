...
def normalize_weights(weights: Dict = None) -> Dict:
    """Return simulator weights, accepting raw or wrapped calibration JSON."""
    if weights is None:
...
def _value(entry: Dict, key: str, default: float) -> float:
    """Read a numeric MLWD field, treating null/missing values as defaults."""
...
def _get_mlwd_entry(mlwd_data: Dict, batch_size: int, seq_len: int, phase: str) -> Dict:
    ...
    for k, v in mlwd_data.items():
        if not k.endswith(f"_{phase}"):
            continue
        b = _value(v, "batch_size", 0)
        s = _value(v, "seq_len", 0)
...
def _estimate_alpha_d(mlwd_data: Dict, weights: Dict, decode_bs: int, prefill_len: int) -> float:
    ...
    sigma_bs = _value(decode_entry, "sigma_bs", 0)
    sigma_cu = _value(decode_entry, "sigma_cu", 0)
    sigma_l2 = _value(decode_entry, "sigma_l2", 0)
    sigma_bw = _value(decode_entry, "sigma_bw", 0)

    # Aggressor strength (prefill)
    ci_attn = _value(prefill_entry, "ci_attn", 1.0)
    ci_ffn = _value(prefill_entry, "ci_ffn", 1.0)
    l2_attn = _value(prefill_entry, "l2_attn", 0.5)
    l2_ffn = _value(prefill_entry, "l2_ffn", 0.5)
    g_launch = _value(prefill_entry, "g_launch", 100.0)
    r_attn = _value(prefill_entry, "r_attn", 0.3)
    r_ffn = _value(prefill_entry, "r_ffn", 0.5)
    ipc = _value(prefill_entry, "ipc", 0.5)
    ...
    # Overlap factor
    r_decode = _value(decode_entry, "r_attn", 0.3) + _value(decode_entry, "r_ffn", 0.5)
...
def _estimate_alpha_p(mlwd_data: Dict, weights: Dict, decode_bs: int, prefill_len: int) -> float:
    ...
    sigma_bs = _value(prefill_entry, "sigma_bs", 0)
    sigma_cu = _value(prefill_entry, "sigma_cu", 0)
    sigma_l2 = _value(prefill_entry, "sigma_l2", 0)
    sigma_bw = _value(prefill_entry, "sigma_bw", 0)

    ci_attn = _value(decode_entry, "ci_attn", 0.5)
    ci_ffn = _value(decode_entry, "ci_ffn", 0.5)
    l2_attn = _value(decode_entry, "l2_attn", 0.5)
    l2_ffn = _value(decode_entry, "l2_ffn", 0.5)
    g_launch = _value(decode_entry, "g_launch", 100.0)
    r_attn = _value(decode_entry, "r_attn", 0.3)
    r_ffn = _value(decode_entry, "r_ffn", 0.5)
    ipc = _value(decode_entry, "ipc", 0.3)
    ...
    r_prefill = _value(prefill_entry, "r_attn", 0.3) + _value(prefill_entry, "r_ffn", 0.5)
    omega = min(r_decode, r_prefill)
